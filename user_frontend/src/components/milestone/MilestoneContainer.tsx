...
import { Clock, CheckCircle, AlertCircle, Plus, ExternalLink, Github } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const MilestoneContainer = ({ milestones = [], onAddMilestone, projectId }) => {
  ...
  const handleRelease = async (milestoneId) => {
    try {
      ...
      setError(err.response?.data?.message || "An error occurred while releasing payment");
      ...
      <div className="flex flex-col gap-4">
        {milestones.length > 0 ? (
          milestones.map((milestone, index) => {
            const { containerClass, icon, showViewDetails, showRelease } = getStatusProps(milestone.status);
        ...
        <DialogContent className="max-w-xl">
          ...
          {selectedMilestone && (
            <div className="mt-4 space-y-4">
              <div>
                <h3 className="font-medium text-lg">{selectedMilestone.description}</h3>
                <p className="text-gray-600">Status: <span className="font-medium capitalize">{selectedMilestone.status}</span></p>
                <p className="text-gray-600">Amount: <span className="font-medium">${selectedMilestone.amount}</span></p>
              </div>
              ...
              {selectedMilestone.screenshots && selectedMilestone.screenshots.length > 0 && (
                <div className="pt-2">
              ...
              {selectedMilestone.notes && (
                <div className="pt-2">
                  ...
                  <p className="text-gray-700 whitespace-pre-line">{selectedMilestone.notes}</p>
                </div>
              )}
