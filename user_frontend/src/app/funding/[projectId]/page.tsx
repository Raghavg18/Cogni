...
// Initialize Stripe
const stripePromise = loadStripe("pk_test_51R5086GdIuM6VO5RXpUs9eHpwiAyQG9vl7sQmEbB4mGDi2cfDBdNkyGBhsLAosYvoaIdwL5VxeZgii3usmE4MdKq00zuubmgdO");
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4">
      ...
      <button
        ...
        className={`w-full bg-[rgba(79,115,150,1)] hover:bg-[rgba(60,90,120,1)] text-white px-4 py-2 rounded-md ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
      >
...
const SelectFreelancer = () => {
  ...
  const fetchProjectDetails = async (projectId: string) => {
    try {
      ...
      const total = response.data.milestones.reduce((sum, milestone) => 
  ...
  const updateProjectWithFreelancer = async () => {
    try {
      await axios.put(`http://localhost:8000/project/${projectData.project._id}/assign-freelancer`, {
          ...
          <main className="flex w-full justify-center flex-1 h-full px-4 md:px-8 lg:px-40 py-5 max-md:max-w-full">
            <div className="flex min-w-60 w-full max-w-[960px] flex-col overflow-hidden items-stretch flex-1 shrink basis-[0%] max-md:max-w-full">
              <div className="bg-white rounded-lg shadow-sm mt-6 p-3">
                ...
                <PageTitle
                  title={paymentSuccess ? "Payment Successful" : "Select Freelancer"}
                  actionLabel={!paymentSuccess && !showPaymentForm && selectedFreelancer ? "Continue to Payment" : ""}
                  ...
                  <>
                    ...
                    {paymentSuccess ? (
                      <div className="px-6 py-8 text-center">
                        ...
                        <h3 className="text-xl font-bold text-[rgba(13,20,28,1)] mb-2">Payment Successful!</h3>
                        <p className="text-[rgba(79,115,150,1)] mb-4">
                          Your payment of ${totalAmount.toFixed(2)} has been processed successfully.
                        </p>
                        <p className="text-[rgba(79,115,150,1)] mb-4">
                          You have assigned <span className="font-semibold">{selectedFreelancer?.username}</span> to your project.
                        </p>
                      ...
                      <>
                        ...
                        {projectData && (
                          <div className="px-6 py-4 border-b border-[rgba(229,232,235,1)]">
                            <h3 className="text-lg font-semibold text-[rgba(13,20,28,1)] mb-2">Project Summary</h3>
                            <p className="text-[rgba(79,115,150,1)] mb-4">{projectData.project.description}</p>
                            
                            <div className="mb-4">
                              <h4 className="text-md font-medium text-[rgba(13,20,28,1)] mb-2">Milestones</h4>
                              <div className="bg-[rgba(247,250,252,1)] p-3 rounded-md">
                                {projectData.milestones.map((milestone, index) => (
                                  <div key={index} className="mb-2 pb-2 border-b border-[rgba(229,232,235,1)] last:border-0 last:mb-0 last:pb-0">
                                    <div className="flex justify-between">
                                      <span className="font-medium text-[rgba(13,20,28,1)]">{milestone.description || `Milestone ${index + 1}`}</span>
                                      <span className="text-[rgba(79,115,150,1)]">${parseFloat(milestone.amount).toFixed(2)}</span>
                                    </div>
                                  </div>
                                ))}
                        ...
                        {showPaymentForm ? (
                          <div className="px-6 py-4">
                            <h3 className="text-lg font-semibold text-[rgba(13,20,28,1)] mb-4">Payment Details</h3>
                            <div className="bg-[rgba(247,250,252,1)] p-4 rounded-md mb-4">
                              <div className="flex items-center mb-4">
                                ...
                                <div>
                                  <h4 className="font-medium text-[rgba(13,20,28,1)]">{selectedFreelancer.username}</h4>
                                  <p className="text-sm text-[rgba(79,115,150,1)]">Freelancer</p>
                                </div>
                              </div>
                              <p className="text-[rgba(79,115,150,1)] text-sm mb-2">
                                You are about to fund the escrow account for this project.
                              </p>
                              <p className="text-[rgba(79,115,150,1)] text-sm mb-2">
                                The total amount of ${totalAmount.toFixed(2)} will be held in escrow until you approve each milestone.
                              </p>
                            </div>
                          ...
                          <div className="px-6 py-4">
                            <h3 className="text-lg font-semibold text-[rgba(13,20,28,1)] mb-4">Available Freelancers</h3>
                                ...
                                {freelancers.map((freelancer) => (
                                  ...
                                  >
                                    <div className="flex items-center">
                                      ...
                                      <div>
                                        <h4 className="font-medium text-[rgba(13,20,28,1)]">{freelancer.username}</h4>
                                        <p className="text-sm text-[rgba(79,115,150,1)]">Freelancer</p>
                                      </div>
                                      {selectedFreelancer?._id === freelancer._id && (
